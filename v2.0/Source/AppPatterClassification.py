...
for solar_disk in lst_solar_disk:
    for window in solar_disk.lst_windows:
        win_name = db_images.str_report_path+solar_disk.pattern_name+\
            "_win_"+str(window.nro_line)+"_"+str(window.nro_column)+".jpg"
...
for pat_base in lst_patterns:
    ...
    for pos_pattern in range(len(lst_patterns)-pos):
        val_sum         = 0
        pat_reference   = lst_patterns[pos_pattern]
        ...
        if distance <= L2 and distance > 0:
            l2 = distance
            #print ".|__.."+pat_base.pattern_name+"\t"+pat_reference.pattern_name+"...: "+str(distance)
            lst_tmp.append(pat_reference)
    pat_base.setLstNearPatterns(lst_tmp)  
    pat_base.l2 = l2
    name_val = name_val+1
    pat_base.class_name = chr(name_val)
    pos = pos + 1     
...
lst_tmp = 0
for pattern in lst_patterns:
    print ".|...Pattern class len..."+ str(len(pattern.lst_near_patterns))
    print ".|.........Pattern name. "+ pattern.pattern_name
    if len(pattern.lst_near_patterns) > 0:
        for pat in pattern.lst_near_patterns:
            print ".|...................... " + pat.pattern_name
    else:
        print ".|...................... NO REFERENCE"
...
file_name = file_name + ".txt"
file_out = open(db_images.str_report_path+file_name,"w")
str_line = "Class Name \t Patterns <= L2"
if probe_func_list[0]:
